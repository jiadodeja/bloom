...
A small interactive bouquet, built entirely in the browser with no libraries using just SVG, canvas, and CSS.
...
**`heart.js`** fills a heart shape with scattered dots, using the classic implicit heart curve `(x² + y² − 1)³ − x²y³ ≤ 0` to decide which random points land inside it. It waits for the bouquet to finish blooming, then blooms in and settles into a looping heartbeat pulse.
...
Fully static. No server, no dependencies. Open `index.html` in a browser, or visit the live link above.
